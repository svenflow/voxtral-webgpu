...
| `options.maxFrames` | `number` | `500` | Max frames to generate. Each frame ≈ 80ms. 500 = ~40s, 1500 = ~2min, 45000 = ~1hr |
| `options.onFrame` | `function` | — | `(frame, semanticCode, acousticCodes) => void` — per-frame progress |
| `options.onStage` | `function` | — | `(stage, doneMs?) => void` — fires when entering/completing each pipeline stage (`'backbone'`, `'fm'`, `'codec'`) |
...
3. **Codec Decoder (Mimi):** Converts codes to 24kHz waveform via 4-stage upsampling (stride 2×2×2×1) with windowed ALiBi attention and transposed convolutions. Each frame = 1920 samples (80ms).

All three stages run entirely on the GPU via 50+ WGSL compute shaders. No WASM, no ONNX. The codec decoder uses per-layer GPU submissions with a buffer pool to keep peak memory low on laptops.
